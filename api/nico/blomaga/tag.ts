...
export default async function (req: NowRequest, res: NowResponse) {
  ...
  const urls = [
    `https://ch.nicovideo.jp/search/${keyword}?type=article&page=` + calc_sum(_offset_num, 1) +`&sort=${_sort_order}&mode=t`,
    `https://ch.nicovideo.jp/search/${keyword}?type=article&page=` + calc_sum(_offset_num, 2) +`&sort=${_sort_order}&mode=t`,
    `https://ch.nicovideo.jp/search/${keyword}?type=article&page=` + calc_sum(_offset_num, 3) +`&sort=${_sort_order}&mode=t`,
    `https://ch.nicovideo.jp/search/${keyword}?type=article&page=` + calc_sum(_offset_num, 4) +`&sort=${_sort_order}&mode=t`,
    `https://ch.nicovideo.jp/search/${keyword}?type=article&page=` + calc_sum(_offset_num, 5) +`&sort=${_sort_order}&mode=t`,
  ]
